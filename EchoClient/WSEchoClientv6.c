...
void main(int argc, char *argv[])   // argc is # of strings following command, argv[] is array of ptrs to the strings
{
	...
	if (connect(serverSock, (struct sockaddr*)&serverInfo, sizeof(serverInfo)) < 0)    DisplayFatalErr("connect() function failed.");
	...
	if (send(serverSock, phrase, len, 0) != len)	DisplayFatalErr("send() function failed.");
	...
	while (len > 0 && ((bytesRead = recv(serverSock, rcvBuf, RCVBUFSIZ - 1, 0)) > 0)) {
		rcvBuf[bytesRead] = '\0';
		printf("%s", rcvBuf);
		len -= bytesRead;

	}
	if (bytesRead < 0)	DisplayFatalErr("recv() function failed.");
	printf("\n");
